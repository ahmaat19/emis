...
const Labels = () => {
  ...
  const getApi = apiHook({
    key: ['labels'],
    method: 'GET',
    url: `setting/labels?page=${page}&q=${q}&limit=${25}`,
  })?.get
      ...
      {getApi?.isLoading ? (
        ...
        <div className="table-responsive bg-light p-3 mt-2">
          ...
          <table className="table table-sm table-border">
            ...
            <tbody>
              {getApi?.data?.data?.map((item: ILabel, i: number) => (
                <tr key={i}>
                  <td>{item?.name}</td>
                  <td>{item?.module}</td>
                  <td>
...
export default dynamic(() => Promise.resolve(withAuth(Labels)), {
  ssr: false,
})
