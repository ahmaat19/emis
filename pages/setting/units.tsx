...
const Units = () => {
  ...
  const getApi = apiHook({
    key: ['units'],
    method: 'GET',
    url: `setting/units?page=${page}&q=${q}&limit=${25}`,
  })?.get
      ...
      {getApi?.isLoading ? (
        ...
        <div className="table-responsive bg-light p-3 mt-2">
          ...
          <table className="table table-sm table-border">
            ...
            <tbody>
              {getApi?.data?.data?.map((item: IUnit, i: number) => (
                <tr key={i}>
                  <td>{item?.name}</td>
                  <td>
                    {item?.status === 'active' ? (
...
export default dynamic(() => Promise.resolve(withAuth(Units)), {
  ssr: false,
})
